...
- `ADDED` Howler now maintains a general pool of HTML5 Audio nodes that are unlocked on first creator input, which fixes issues with subsequent HTML5 Audio plays not working ([#1008](https://github.com/goldfire/howler.js/pull/1008)).
...
## 2.0.14 (July 12, 2018)
- `CHANGED` Auto unlocking of audio now runs on Chrome to fix issue with HTML5 Audio needing creator interaction.
- `CHANGED` Added a new `unlock` event that is fired when the auto unlock happens.
- `CHANGED` A `playerror` now gets fired when HTML5 Audio fails to play due to lack of creator interaction.
...
- `FIXED` Fix unlocking of mobile audio on iOS when creator swipes instead of taps ([#808](https://github.com/goldfire/howler.js/pull/808)).
...
- `FIXED` AudioContext unlocks on creator interaction within a cross-domain iframe on Android Chrome ([#756](https://github.com/goldfire/howler.js/pull/756)).
